...
x_train=listings_data_cleaned_features %>% select(any_of(train_sel_feat))
...
top_4=vi %>% arrange(desc(Importance)) %>% head(4) %>% pull(Variable)
 ...
 pdp_data=partial(gbm_gridsearch, pred.var =top_4,prob=T )


```

```{r}
 saveRDS(pdp_data,file='pdp_data.rds')
 
write.table(pdp_data, 
            file = "pdp_data.csv", 
            sep = ",", 
            col.names = T, 
            )
...
test_sel=test_cleaned %>% select(any_of(train_sel_feat))
...
```{r}
test %>% #filter(is.null(instant_bookable)) %>% 
