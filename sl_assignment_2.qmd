...
nrow(listings_data);ncol(listings_data)
```

# Introduction: Exploartory Data Analysis

The training data consits of 10 rows of observations across 25 features. The the data is significantly imbalanced in favour of the target class at with \~81% of observations in the filly_booked_30='no' class.\
...
#| warning: false
#| fig-subcap: 
#|   - "Spatial Plot of BNB Listings"
#|   - "Fully Booked 30 Label Distribution"
#| layout-ncol: 2
...
a <- listings_data_cleaned %>% 
  ggplot(aes(x = fully_booked_30, 
             y = host_since_months_int, 
             fill = fully_booked_30)) +
  geom_boxplot() +
  labs(
    title = "Host Age By 30d Fully Booked Status",
    x = "30-day booked status",
    y = "Host age (months)",
    fill = "30 booked status"
  ) +
  theme_minimal()
