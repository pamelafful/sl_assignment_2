...
#| fig-height: 8
#| fig-width: 15
#| fig-cap: "Averages and Distributions of Features per Target Class "
...
#| fig-height: 2.5
#| fig-width: 6
#| fig-cap: "Initial Decision Tree Feature Splits "
...
#| message: false
#| warning: false
#| tbl-cap: "Vanilla Logistic Regression Summary"
...
![](vanilla_regress_summ.png){fig-align="center" width="479" height="465"}
...
-   Significant Features:
    ...
    -   Despite the number review being positive, the actual ratings does not show a significant effect the the log likliehood of the target class.
...
-   Notable Insignificant Features:

    -   Most interestingly the price did not show a stritically significant effect on the (log) lilkiehood of the target class ( although this was a key feature used the decision tree above).\

**L1 Regularization**

L1 Regularization with using K-folds validation is now trained on the data with AUC used as the cross validation metric.

```{r regul, eval=T, echo=F}
#| message: false
#| warning: false
#| fig-cap: "CV AUC Scores Per Log Lambda Values"
...
plot(logmod_l1, main = "")
title(main = "L1 Regularization CV AUC Scores", line = 3)
...
The best CV AUC score is obtained at at 0.018 with associated 6 non-zero coefficients ( out of the 25) retained by the model. \
Most notably:

-    The number reviews per month maintains its position as having the biggest marginal impact on the (log) likelihood of the target.

-   Location variables, location and longitude are penalized to zero.

-   For the categorical feature, host_response, the host_response_time_within_a_day was the only retained category in the regression

-   Minimum_nights_avg_ntm was not penalised in the L1 regression even though it was not significant in the vanilla model\

```{r zero_coeff, eval=T, echo=FALSE}
#| message: false
#| warning: false
#| fig-height: 8
#| fig-width: 8
...
best_coeff_tbl=best_coeff %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "L1 Logistic Regression Results, penalized variables in red"
  ) %>%
  row_spec(
...
![](L1_regress_summ.png){width="509" height="442"}\

**Random Forest**

```{r,rf_weights, echo=FALSE,eval=T}
...
```{r rd_res, eval=T, echo=F}
#| message: false
#| warning: false
#| fig-width: 6.7
#| fig-cap: "CV Accuracy Scores vs Hyparameters"
...
The best CV accuracy score is achieved at the parameters shown above.

**Boosting Model**

The gbm model is trained on the non-zero features from the L1 regularized model to speed up training, as it has been reasonably established that the majority of these features are not meaningful to the model.

Also, a custom log-loss function is used as the primary metric for the cross-validation training- thus the hyper parameters that yield the CV lowest log loss function are chosen as the best during training.
...
```{r gbm_hp_sel, eval=T, echo=F}
#| message: false
#| warning: false
#| fig-cap: "GBM CV Log Loss Values vs Hyperparameters"
#| fig-height: 6


plot(gbm_gridsearch,main = "GBM CV Log Loss Scores")

```

The best hyper parameters is acquired at the hyperparameter combinations below:\
...
#| fig-height: 8
#| fig-width: 8
#| fig-cap: "Cross Model Cross Validation Metrics"
# concat results
 library(patchwork)
...
-   Lowest penalty of negative predictions from log loss which is the primary metric, that is the model is able to correctly classify the positive class with a high degree of confidence.

-   Using the fraction of positive class observations available in the data, \~81%, all of the models report higher AUC scores relative to the baseline, although tree models, RF and GBM models report near perfect AUC scores (close to 1).

-   For actual prediction-based metrics, Specificity, Precision, F1 and Accuracy, higher scores are observed for the RF models all across. In particular, CV and F1 scores are significantly higher \* add model explanation/interpretaion here

Given the above, it is most important that a model is able to make predictions which a relatively high confidence, essentially better than a coin toss. Although count-based metric results are important for overall performance, for particularly imbalanced data as this, the dregree of confidence/probability becomes esapacially significant for positive/target class predictions. As such, the GBM model is selected as the the best model. It has the best probability-based metrics as well as the reasonably good count-based metrics.

```{r,gmb_roc, eval=T, echo=F}
#| message: false
#| warning: false
#|fig-cap: "GBM CV AUC Score"
library(ROCR)
library(pROC)
library(AUC)
...
The ROC curve shows that model performance of the GBM model is very near to the perfect case (PRAUC=1) for the given cut-off threshold. The model produces a very high true positive rate ( i.e is able to classify the postive class with a high degree of accuracy), with a very low false positive rate. It is also significantky better tan the worst case scenario coin-flip.
...
```{r feat_imp, eval=T, echo=F}
#| message: false
#| warning: false
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "GBM Feature Importance"
# Answer rest of q3
