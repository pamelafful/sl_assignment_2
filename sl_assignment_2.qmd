...
The best CV accuracy score is achieved at the parameters shown below

**Boosting Model**

The gbm model is trained on the non-zero features from the L1 regularized model to speed up training,, as it has been reasonably established that the majority of these are not meaningful to the model.

Also, a custom log-loss function is used as the primary metric for the the cross validation training- thus the hyper parameters that yield the lowest log loss function are chosen as the best during training.
